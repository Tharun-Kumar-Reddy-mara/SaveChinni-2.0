...
public class RobotManager : MonoBehaviour
{
    ...
    [SerializeField] private Transform [] lilly_Spawn_Points;
    [SerializeField] private Transform [] player_Spawn_Points;
    ...
    private void Awake()
    {
        ...
        lilly = GameObject.FindGameObjectWithTag("Lilly");
        random = Random.Range(0,lilly_Spawn_Points.Length);
    }
    ...
    private void SpawnLilly(){
        if(random == 1 || random == 0){
           lilly.transform.localRotation = Quaternion.Euler(0,90f,0f);
        }
        else if(random == 2){
           lilly.transform.localRotation = Quaternion.Euler(0,0f,0f);
        }
        else{
           lilly.transform.localRotation = Quaternion.Euler(0,180f,0f);
        }
        lilly.transform.position = new Vector3(lilly_Spawn_Points[random].position.x,lilly_Spawn_Points[random].position.y,lilly_Spawn_Points[random].position.z);
    }
    private void SpawnPlayer(){
        if(random == 0 || random == 1){
           transform.localRotation = Quaternion.Euler(0,90f,0f);
        }else if(random == 2){
           transform.localRotation = Quaternion.Euler(0,180f,0f);
        }
        transform.localPosition = new Vector3(player_Spawn_Points[random].position.x,player_Spawn_Points[random].position.y,player_Spawn_Points[random].position.z);
    }
